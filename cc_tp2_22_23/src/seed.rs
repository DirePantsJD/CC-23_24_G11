...
pub fn upload() {
    let server_socket: Arc<UdpSocket> = Arc::new(
        UdpSocket::bind(
            "0.0.0.0:".to_owned() + &fsnp::SERVER_UDP_PORT.to_string(),
        )
        .unwrap(),
    );
    let send: Arc<Mutex<bool>> = Arc::new(Mutex::new(true));
    let packets: Arc<Mutex<Vec<([u8; 1500], usize, SocketAddr)>>> =
    ...
    for _ in 0..MAX_NUMBER_THREADS {
        ...
        thread::spawn(move || {
            worker(packets_clone, ch.1, send_clone, ssocket_clone)
        });
    ...
    loop {
        ...
        channels
            .iter()
            .for_each(|sender| sender.send(true).unwrap());
    }
}
...
fn worker(
    ...
    loop {
        ...
        if recvch.recv().unwrap() {
            ...
            if work {
                if let Ok(mut request) = fsnp::Protocol::read_packet(
                    &request_raw.0,
                    request_raw.1 as u16,
                ) {
                    let bytes_read = read_file(
                        (String::from("./shared/") + request.filename.trim())
                            .as_str(),
                        request.chunk_id,
                        &mut chunk_data,
                    )
