...
const MAX_PACKET_SIZE:usize = 1453;

pub struct FSNode_Protocol<'a>{
    action:u8,
    chunk_id:u32,
    filename:&'a str,
    len_chunk:u16,
...
impl< 'a> FSNode_Protocol<'a>{
    
    pub fn read_packet(packet:&'a [u8]) -> anyhow::Result<FSNode_Protocol> {
    ...
    pub fn build_packet(&self) -> Option<([u8;MAX_PACKET_SIZE],u8)>{
        ...
        let len_chunk_data:[u8;2] = (self.chunk_data.len() as u16).to_le_bytes();
        let filename:&[u8] = self.filename.as_bytes();

        let packet_len:u8 = 1+4+1+2 + self.filename.len() as u8 + self.chunk_data.len() as u8;
        ...
        overwrite_array(8+len_filename.len(),8+len_filename.len()+self.chunk_data.len()-1,&self.chunk_data,&mut packet);
